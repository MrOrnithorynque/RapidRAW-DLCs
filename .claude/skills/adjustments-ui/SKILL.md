...
- `ControlsPanel` maps `Object.keys(ADJUSTMENT_SECTIONS)` to a fixed `{ basic, curves, color, details, effects }` component map; the same map is reused in `MasksPanel` for per-mask sections. JSON keys are camelCase. They are **not** serde-deserialized into the Rust structs: `GlobalAdjustments`/`MaskAdjustments` are `#[repr(C)]` GPU buffers, and each camelCase key is read manually field-by-field in `get_global_adjustments_from_json` / `get_mask_adjustments_from_json` (`src-tauri/src/image_processing.rs`) — see `image-pipeline`/`gpu-shaders`.
...
- Per-mask sections do NOT call the global `setAdjustments`. `MasksPanel` passes a local `setMaskContainerAdjustments` wrapper that routes through `updateContainer(id, { adjustments })` so writes land in `adjustments.masks[i].adjustments`. AI patches are different: an `AiPatch` has **no** adjustment section (no `adjustments` field) — it holds generative-inpaint data (`prompt`/`patchData`/`subMasks`) in `adjustments.aiPatches[i]`, written through the global `setAdjustments` in `AIPanel.tsx`.
...
3. Backend (see `gpu-shaders` + `image-pipeline`): add `pub bloom: f32` to `GlobalAdjustments` (and `MaskAdjustments`) in `src-tauri/src/image_processing.rs`, keeping the `repr(C)`/bytemuck padding alignment intact; default it; **add a `get_val("effects", "bloom", SCALE, default)` extraction line in `get_global_adjustments_from_json` (and `get_mask_adjustments_from_json`)** — the struct is not auto-deserialized, so without this line the camelCase key is silently ignored and the field stays at its default; consume it in `src-tauri/src/shaders/shader.wgsl`.
4. i18n: add `adjustments.effects.bloom` to `src/i18n/locales/en.json` then run `npm run i18n:extract`.
